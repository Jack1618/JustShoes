<?php
  ...
  if(isset($_POST["id_utente"])){
    ...
    $attivo = $_POST["attivo"];
    $mysqli->query("UPDATE Utente SET attivo = $attivo WHERE id_utente=$id_utente");
  ...
  if(isset($_POST["cerca"])){
    $nome = $_POST["cerca"];
    $utenti = $mysqli->query("SELECT * FROM Utente WHERE id_gruppo_applicativo = 2 AND email LIKE '%$nome%'");

  }
  else{
    $utenti = $mysqli->query("SELECT * FROM Utente WHERE id_gruppo_applicativo = 2");
  }
?>
