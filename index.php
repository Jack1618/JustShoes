...
<?php
	$sqlScontate = "SELECT id_scarpa, Scarpa.nome as 'nome', prezzo, sconto, foto, Marca.nome AS 'marca'  FROM Scarpa JOIN Marca ON Scarpa.id_marca = Marca.id_marca  WHERE sconto > 0 ORDER BY sconto ASC LIMIT 4";

	$sqlTopSeller = "SELECT Scarpa.id_scarpa, Scarpa.nome as 'nome', prezzo, sconto, foto, Marca.nome AS 'marca' FROM Scarpa  JOIN (SELECT id_scarpa, SUM(quantita) as tot FROM Dettagli_Acquisto GROUP BY id_scarpa) AS qtaToT ON Scarpa.id_scarpa = qtaTot.id_scarpa JOIN Marca ON Scarpa.id_marca = Marca.id_marca ORDER BY tot ASC LIMIT 4";
